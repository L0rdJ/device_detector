 ...
 */
/******************************************************************************
Name:    cacheengine.mysqlpdo.class.php
Version: 2.7.0
Config:  default
...
if (class_exists("DetectRight")) {
	DetectRight::registerClass("DRMySQLPDOCache");
}
 ...
 * @internal
 */
Class DRMySQLPDOCache extends DRCache {
  ...
  UNIQUE KEY `key_UNIQUE` (`key`,`serialized`)
	...
	function cache_delete($key) {
		...
		$key = $this->_normalize($key);
		try {
			@$this->cache->exec("delete from ".self::$table." where key = '$key'");
			return true;
		} catch (Exception $e) {
			return false;
		}
	...
	function cache_init($params) {
		...
		if (!extension_loaded("PDO")) {
			throw new DetectRightException("PDO not installed");
		} 
		...
		try {
			$cache = new PDO("mysql:host=$host;port=$port;dbname=$database",$user,$password);
			$this->cache = $cache;
			$this->cacheOK = true;
		} catch (Exception $e) {
			throw new DetectRightException("Failed to open mysql PDO",$e);
		}
	...
	function cache_flush() {
		if (!$this->cache_ok()) return false;
		try {
			$this->cache->exec("delete from ".self::$table);
		} catch (Exception $e) {
...
if (!class_exists("MySQLPDOCache")) {
	DetectRight::registerClass("MySQLPDOCache");
	
	Class DRMySQLPDOCache extends DRMySQLCache {
